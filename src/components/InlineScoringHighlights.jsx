 ...
 * "expand" button hands off to the caller with the current play index *and*
 * the seconds elapsed inside that clip so the full modal can resume at the
 * same timestamp instead of restarting the video.
 */
export default function InlineScoringHighlights({
  ...
  // video-player state, but the countdown mirrors the clip length so
  // (PLAY_DURATION - timeRemaining) is close enough to hand off to the
  // modal as a resume offset.
  const handleExpand = () => {
    if (!onExpand) return
    const elapsed = Math.max(0, PLAY_DURATION - timeRemaining)
