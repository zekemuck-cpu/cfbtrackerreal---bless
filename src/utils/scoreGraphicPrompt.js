...
Either way the result is a full-bleed photographic background in team colors. Do NOT use a flat solid-color rectangle.`
  ...
  const lines = [
    ...
    `2. "${winText}" — bold all-caps label near the top`,
    `3. ${featuredName} primary logo/wordmark — large and prominent, the hero brand mark`,
    `4. Final score block — show both teams with their logos and scores, scoreboard style:`,
    `     [${featuredName} logo]  ${sf}  —  ${so}  [${oppName} logo]`,
    `   The featured team's logo is larger; the opponent's is smaller but clearly visible.`,
    `   Both team names or logos must appear paired with their respective score numbers.`,
    `   The featured team's score (${sf}) is the dominant number — biggest, highest contrast.`,
    `   The opponent's score (${so}) reads clearly but is visually subordinate.`,
    `5. "${oppRankLabel}${oppName}" as a readable text label near their score`,
    `6. "${gameLabel}${year ? ` · ${year}` : ''}" and "${statsLine}" — small caption at bottom`,
    `7. Thin rules, corner marks, or score dividers in ${secondary}`,
    ``,
