/**
 * Stale-chunk recovery helpers — shared by the global handlers in main.jsx
 * and RouteErrorBoundary.
 *
 * When Vercel redeploys, the hashed asset filenames change. A tab left open
 * on the old build tries to lazy-load chunks that no longer exist; the
 * browser either gets a 404 or Vercel's SPA fallback (index.html) back, and
 * dynamic import rejects with "Failed to fetch dynamically imported module".
 *
 * We probe the failing URL before reloading: 404 or HTML response → genuine
 * stale, reload; anything else → transient, log and continue. A short-lived
...
export function extractUrlFromError(err) {
...
export async function reloadIfStale(url) {
