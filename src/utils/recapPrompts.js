...
//
// Algorithm: base map + per-team overrides — mirrors getCustomConferencesForYear
// in DynastyContext.jsx exactly. Inlined here because the prompt utility
// deliberately has no React/context dependency.
//
//   Step 1 — Base map (covers all teams; handles bulk saves):
//     customConferencesByYear[year] → walk-back → customConferences → null
//   Step 2 — Per-team overrides (handle individual team edits):
//     conferenceByTeamYear[abbr][year]  (lower priority)
//     teams[tid].byYear[year].conference  (canonical, wins)
//   Step 3 — Apply overrides to clone of base; return.
//
// WHY NOT BUILD FROM PER-TEAM DATA ALONE:
//   Many dynasties have conference data ONLY in the bulk stores. If a
//   single team was individually edited (byYear.conference set for 1 team),
//   building from per-team alone would silently drop all other teams.
//   The overlay approach is always safe.
// ---------------------------------------------------------------------------
...
export function buildWeekRecapPrompt(dynasty, year, week, opts = {}) {
  ...
  return [
    ...
    `═══════════════════════════════════════════════════════════`,
    `Before you write a single sentence, scan the data for the week's real narratives. The best recap leads with the story that matters most — not just the highest rank, but the most consequential result. Scan for:`,
    ``,
    `  • A defending national champion: if they played this week, their result is mandatory coverage. A defending champ's loss is always one of the top two stories regardless of their current rank.`,
    `  • Any top-5 team that lost — especially to an unranked team.`,
    `  • A ranked team beaten by a significantly lower-ranked or unranked opponent.`,
    `  • A team that has fallen dramatically from a preseason ranking (check PRIOR-YEAR CONTEXT).`,
    `  • An unbeaten team still standing late in the season.`,
    `  • A conference race that just tightened or broke open.`,
    `  • A rivalry game, a rematch (check LAST MEETINGS), or a game with historic stakes.`,
    ``,
    `Rank these by consequence. The story at the top of your list leads the recap. If you find the week's biggest result buried in the middle of your draft, rewrite.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `VOICE — ESPN WEEKEND WRAP, NOT COLUMNIST`,
    `═══════════════════════════════════════════════════════════`,
    `You are REPORTING the week, not arguing about it. Your draft should read like the front page of ESPN.com on a Sunday morning — news-forward and fact-driven — not like an 1,800-word column on The Athletic.`,
    ``,
    `✅ WHAT GOOD SOUNDS LIKE: "Missouri knocked off No. 3 Georgia 27-24 on the road Saturday, the Bulldogs' first loss of the season and a result that scrambles the SEC race. Quarterback Devon Hill passed for 312 yards and two scores for the Tigers, who improved to 7-2. Georgia dropped from No. 3 to No. 8 in the latest poll."`,
    ``,
    `- Lead with the biggest result — what happened, who beat whom, score — not a clever thesis.`,
    `- Active voice, past tense. After first reference, last names only for players.`,
    `- One precise verb beats two adverbs.`,
    `- Skip throwaway transitions: "Meanwhile," "Elsewhere," as paragraph openers.`,
    ``,
    `BANNED VERBS — use any AT MOST TWICE across the whole recap: rolled, drilled, flattened, crushed, edged, topped, hammered, handled, dropped (as in "dropped 52 on"). Vary by what the data shows: "controlled from the opening drive", "pulled away in the fourth", "survived in [city]", "held on", "never trailed".`,
    ``,
    `BANNED PHRASES — do NOT write these. They are the AI tells that turn an ESPN wrap into a try-hard column:`,
    `   ✗ "did not X so much as Y" — column thesis opener`,
    `   ✗ "[Team] survives but does not separate" — thesis subhead`,
    `   ✗ "the cleanest version of their case" / "the cleanest possible response"`,
    `   ✗ "perfection and momentum are not always the same thing"`,
    `   ✗ "[Month] punishes teams that..." / "[Month] rewards teams that..."`,
    `   ✗ "the bracket drew itself" / "the committee just inherited"`,
    `   ✗ "[Team]'s season died in [city]" / "playoff path closed"`,
    `   ✗ "set the tone", "imposed their will", "found their rhythm", "made plays when it mattered", "sealed it", "all the momentum", "answered the call", "leaves no doubt"`,
    `   ✗ "data point" / "data points" — corporate AI-speak; write what actually happened`,
    `   ✗ "by ranking" as a qualifier (e.g. "biggest upset by ranking", "largest margin by points") — unnatural phrase; just describe what happened`,
    `   ✗ "won another high-scoring game" / "in another high-scoring affair" — invents a pattern not in the data`,
    `   ✗ Any closing sentence whose only job is to be clever — end on a FACT.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `RULE A — LEDE NAMES THE STORY`,
    `═══════════════════════════════════════════════════════════`,
    `First 1-2 sentences: name the week's biggest result, the score, and what it changed (record, ranking, conference race, streak). Inverted pyramid. The lede REPORTS — it does not argue.`,
    ``,
    `LEDE HIERARCHY — which game earns the lead:`,
    `  1. Highest-ranked team to LOSE this week — especially if it fell to an unranked opponent.`,
    `  2. Any ranked team losing to an unranked team (regardless of margin).`,
    `  3. The game between the two highest-ranked teams when no upsets occurred.`,
    `  4. Defending national champion result, even if they're currently unranked.`,
    ``,
    `✅ "Missouri knocked off No. 3 Georgia 27-24 Saturday, the Bulldogs' first loss and Georgia's biggest stumble of the season." (unranked beats top-5 → leads)`,
    `✅ "Tennessee beat Texas 38-32 in a top-10 matchup Saturday that tightened the SEC race." (highest-ranked game → leads when no upset)`,
    ``,
    `FORBIDDEN LEDE PATTERNS:`,
    `   ✗ Any opening that argues an abstract claim instead of naming a result.`,
    `   ✗ "Saturday saw a slate of..." / "Saturday produced..." (passive announce)`,
    `   ✗ "The Big Ten did not settle anything in Week N so much as expose..." (column thesis)`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `RULE B — CONTEXT OVER SCOREBOARD`,
    `═══════════════════════════════════════════════════════════`,
    `A score is a fact. The STORY is what the score means. For every game you cover in more than one sentence, answer the "so what?" — what did this result change?`,
    ``,
    `WHAT "SO WHAT?" LOOKS LIKE IN PRACTICE:`,
    `   ✗ "Kentucky beat Tennessee 27-21." — a score`,
    `   ✅ "Kentucky beat Tennessee 27-21, handing the Volunteers their second ranked loss and likely pushing them out of the top 10 for the first time this season." — the score AND what it means`,
    ``,
    `   ✗ "Georgia improved to 9-1." — a stat`,
    `   ✅ "Georgia's 9-1 record keeps them in the driver's seat in the SEC East, a game ahead of Tennessee with two conference games left." — the stat with stakes`,
    ``,
    `WHAT TO FRAME for a featured game:`,
    `   • What does the WIN mean? Did it clinch something? Keep a streak alive? Put them in the CFP picture?`,
    `   • What does the LOSS mean? Does it knock them out of the conference race? Drop them from the top 10? End their unbeaten run?`,
    `   • What was UNUSUAL? Lopsided score between ranked teams? A road win against a top-5 opponent? A team everyone wrote off pulling the upset?`,
    `   • Is there a STREAK or milestone? "first ranked win of the season", "held an opponent under 20 for the fourth straight week", "extended their losing streak to 4"`,
    ``,
    `CHOOSE DEPTH OVER BREADTH. Cover 10-15 games with real context — not 40 with nothing but names and scores.`,
    ``,
    `SCORE-DUMP WARNING: Three or more results in a row where each sentence is only "Team A beat Team B, N-N" is a score dump — the most common AI failure in recap writing. If you're listing 4+ results in sequence, either group by theme (conference races, ranked matchups, upsets) or pick fewer and write them with stakes.`,
    ``,
    `When you cover the ranked teams, it is NOT a scoreboard. Do NOT march through the poll in numerical order. Instead: group quiet wins together in one clause and spend your words on the results with real implications — close scores, conference standings, CFP picture. If five ranked teams each won comfortably, that's ONE sentence. "Miami, Notre Dame, Oregon, Georgia and Penn State all won by double digits, with all five now unbeaten or one loss." Do not give each a dedicated sentence.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `RULE C — PRIOR-YEAR / COACHING CONTEXT`,
    `═══════════════════════════════════════════════════════════`,
    `If a featured team finished top-10 last year or played in the CFP, that context can make a result land harder. Use it when it genuinely tightens the story — not as boilerplate filler. "A year after losing the title game, Georgia is back to 9-1." Don't thread "a year removed from..." through every paragraph.`,
    ``,
    `HARD CAP: Use prior-year context for no more than 3-4 teams in the entire recap. Choose the instances where it genuinely changes how the reader understands the result — a defending champion struggling, a preseason No. 1 now unranked, a team back in the top 10 after a rough prior year. Do NOT append prior-year context mechanically to every team you mention, especially in Around the Rankings.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    ...
    `═══════════════════════════════════════════════════════════`,
    `HEADLINE — write an ESPN.com news headline. Sentence case (capitalize the first word and proper nouns). 6-12 words. Reports the week's biggest result. No clever twist, no thesis, no oracle.`,
    ``,
    `✅ "Washington beats Oregon 43-36 to reshuffle the CFP picture"`,
    `✅ "${weekLabel}: Washington upsets Oregon; Clemson, Notre Dame stay perfect"`,
    ...
    `✗ Any headline that argues instead of reports.`,
    ``,
    `SUBHEADS — OPTIONAL, and varied. Many recaps need none; clean prose is often better. When you do use one, keep it a short factual label tied to THIS week's content — never a verb+thesis or comma-twist contrast, and never the same stock set ("Top games / Around the rankings / Around the country") recap after recap.`,
    `   ✅ (sparingly) "Washington 43, Oregon 36"   ✅ "The playoff race"`,
    `   ✗ "Washington Takes the Week, Oregon Pays for It"   ✗ the identical labeled stack on every recap`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    `SELF-CHECK BEFORE SENDING`,
    `═══════════════════════════════════════════════════════════`,
    `Re-read top to bottom. If any check fails, rewrite before sending.`,
    ``,
    `   1. LEDE: does the first sentence name the biggest result (who beat whom, score, what changed)? Not a thesis, not "Saturday saw...", not an abstract claim?`,
    `   2. STORY HIERARCHY: if any top-5 team lost to an unranked opponent this week, is that the lead — not buried in paragraph 4?`,
    `   3. BANNED PHRASES: did you write any of the column-mode phrases above? ("did not X so much as Y", "the cleanest version", "perfection and momentum", "[Month] punishes", "season died in [city]", "bracket drew itself", "data point", "by ranking", etc.)`,
    `   4. SCORE DUMP / CONTEXT CHECK: any stretch of 3+ consecutive score-only sentences? If yes, group by theme or cut to fewer games with more context. For every game you wrote more than one sentence about, ask "did I say what this result MEANS?" — not just what happened.`,
    ...
    ``,
    `WRITING RULES:`,
    `- PUNCTUATION: never use em dashes (—) anywhere in the prose. Use a comma, colon, parentheses, or a separate sentence instead. This is a hard rule.`,
    ...
    `═══════════════════════════════════════════════════════════`,
    `The recap's LENGTH must match how many games the week actually had. Don't pad a quiet week to hit a number, and don't force structure the data can't fill.`,
    ``,
    `• Loaded week (10+ games, several ranked teams played): ~500-800 words. Plenty to work with — pick the dozen-plus games with the most to say and write them with texture.`,
    ``,
    `• Light week (3-9 games, few or no ranked teams): ~300-500 words. If no ranked teams played, don't manufacture a rankings passage by re-describing the standings as if they were news. If poll movement only happened in EARLIER weeks, leave it out — old shifts aren't this week's story.`,
    ``,
    `• Single-game week (Army-Navy, a bye, Week 0): 2-4 short paragraphs on the one game — result, records now final, the context the data gives (rivalry, both teams' season arc, prior-year postseason if relevant). Then stop. Don't pad with "the rest of the top 25 hasn't changed" or last week's poll shifts. The week IS the one game.`,
    ``,
    ...
    `HOW TO STRUCTURE IT`,
    `═══════════════════════════════════════════════════════════`,
    `Use H2 subheads (##) to divide the recap into named sections. Any week with 5+ games needs at least two labeled sections — subheads help the reader navigate and each one frames a distinct storyline.`,
    ``,
    `VARY THE LABELS, not the count. The same four headings every week ("Top games / Around the rankings / Around the country") is a form letter. Name each section for what it's actually covering THIS week:`,
    `   ✅ "## Oklahoma stumbles in Stillwater"   ✅ "## The playoff race"   ✅ "## Big Ten dominates"   ✅ "## Around the country"`,
    `   ✗ "## Top games" used as the lead section label every single week`,
    ``,
    isBowlWeek ? [
      `BOWL WEEK STRUCTURE:`,
      `  1. ${cfpRoundLabel ? cfpRoundLabel.toUpperCase() : 'CFP GAMES'} — MANDATORY LEAD SECTION. Name the subhead for the specific outcome (e.g. "## Georgia survives in the quarterfinals"). Cover every CFP game with full detail: result, both teams' records, what the win means, who advances.`,
      `  2. BOWL RESULTS — the regular (non-CFP) bowl games. Lead with ranked matchups, upsets, or blowouts ≥21 pts. Group unremarkable results together. A single labeled subhead per notable cluster is fine.`,
      `  3. POLL MOVEMENT — if (and only if) the EVOLUTION section shows a clear shift THIS week per RULE D. Skip if nothing changed.`,
      `  Optional: a stat leader line, what's-next (only when the latest snapshot is labeled "POST-${weekLabel.toUpperCase()}").`,
    ].join('\n') : [
      `REGULAR-SEASON STRUCTURE:`,
      `  1. LEAD SECTION — the week's biggest story, named for it. Cover the most consequential result per LEDE HIERARCHY in full: score, what it means for the loser (conference race, ranking, CFP picture, streak), what it means for the winner. Use a specific subhead.`,
      `  2. THE REST OF THE RANKED LANDSCAPE — other ranked teams' results worth noting. Group routine wins in one clause. For any result with actual stakes (a team clinching something, a team losing two straight, a blowout over a quality opponent), write one sentence of context beyond just the score.`,
      `  3. POLL MOVEMENT — if (and only if) there's a real shift THIS week per RULE D. Skip if nothing changed or if movement only happened in prior weeks.`,
      `  4. AROUND THE COUNTRY — notable unranked games: upsets, blowouts ≥30, rivalry results, conference implications. For each: one sentence of score, one sentence of meaning. Selective — not every game.`,
      `  Optional: a standout stat leader with season context, conference-race standings, or a one-line what's-next (only when the latest poll is labeled "POST-${weekLabel.toUpperCase()}").`,
    ].join('\n'),
    ``,
    `CRITICAL regardless of structure: any conference label MUST be the one shown in parens beside the team in that game's data line — NEVER real-world memory ("Hawaii (Pac-12) 35, Boise State (Pac-12) 14" → write "Pac-12", not "Mountain West"). The defending champ's result is mandatory coverage if they played this week, regardless of their current rank.`,
    ``,
    `STILL TRUE WHATEVER SHAPE YOU CHOOSE:`,
    `- Mention each game only ONCE — never circle back to the same result later as filler.`,
    `- No hedging language ("the picture remains a watch list..."), no filler ("showed great heart"), no restating the same scoreline more than once.`,
...
export function buildPreseasonRecapPrompt(dynasty, year) {
  ...
  return [
    ...
    ``,
    `• Em-dashes: NEVER use an em dash (—). Not once. AI defaults to em-dashes for every secondary clause; use a comma, colon, parentheses, or a separate sentence instead.`,
