...
export function buildWeekRecapPrompt(dynasty, year, week) {
  ...
  return [
    ...
    ``,
    `═══════════════════════════════════════════════════════════`,
    `RULE B — DEPTH OVER BREADTH`,
    `═══════════════════════════════════════════════════════════`,
    `A good recap covers 15-20 games with real texture — not 40 games with nothing but names and scores.`,
    ``,
    `Choose the games that have the most to say: ranked matchups, upsets, rivalry context, records on the line, conference implications. Write them with enough detail to be worth reading. Drop the rest.`,
    ``,
    `SCORE-DUMP WARNING: Three or more results in a row where each sentence is only "Team A beat Team B, N-N" is a score dump — the most common AI failure in recap writing. If you're listing 4+ results in sequence, either group by theme (conference races, ranked matchups, upsets) or pick fewer and write them better.`,
    ``,
    `When you cover the ranked teams, it is NOT a scoreboard. Do NOT march through the poll in numerical order giving each team its own score-plus-record sentence. Instead: group quiet wins together in one clause and spend your words on the results with actual texture — close scores, conference implications, notable records. If five ranked teams each won comfortably over weak opponents, that's ONE sentence: "Miami, Notre Dame, Oregon, Georgia and Penn State all won by double digits, with all five now unbeaten or one loss." Do not give each a dedicated sentence with score and prior-year context attached.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    ...
    ``,
    `═══════════════════════════════════════════════════════════`,
    `HOW TO STRUCTURE IT — VARY THIS, WEEK TO WEEK`,
    `═══════════════════════════════════════════════════════════`,
    `There is NO fixed section template, and you must NOT reach for the same headings every week. The stock stack — "Top games" → "Around the rankings" → "Around the country", same order, every single recap — is exactly what makes these read like a generated form letter. Break it.`,
    ``,
    `Let the week decide the shape. One dominant story (a top-5 upset, a title game)? Open on it and let the piece fan out — often 2-4 flowing paragraphs with NO subheads at all. A loaded slate? Group results the way the stories actually cluster this week, and use a subhead only if it genuinely helps — named for THIS week's content, not from a stock list. Vary it: if last week had three subheads, this week might have none or one. Most weeks read best as clean prose with few or no subheads — never four labeled buckets. Write like a person catching a friend up on the weekend, not like filling out a form.`,
    ``,
    isBowlWeek
      ? `This week, the ${cfpRoundLabel || 'CFP'} games are the dominant story: open on them and what they set up (who advanced, seeding, the next round), then fold in the most notable bowls — ranked teams, upsets, blowouts. As prose, not a CFP/BOWL/POLL template.`
      : `Open on the week's biggest result (per the LEDE HIERARCHY), then follow the stories that actually have texture.`,
    ``,
    `Angles worth considering — a MENU, not a checklist, and not headings to print verbatim: the biggest games (ranked-vs-ranked, top-10 losses, upsets; the defending champ's result is mandatory if they played; note rematches from LAST MEETINGS and real prior-year context where it lands); the other ranked teams worth a mention (group the routine wins into a clause, skip the rest — don't march the poll in numerical order); poll movement (only a real shift THIS week, and only as a plain stat — RULE D); notable games beyond the rankings (upsets, blowouts ≥30, rivalry/conference stakes, one-score thrillers). Pull only the threads the week actually gives you.`,
    ``,
    `CRITICAL regardless of shape: any conference label MUST be the one shown in parens beside the team in that game's data line — NEVER real-world memory ("Hawaii (Pac-12) 35, Boise State (Pac-12) 14" → write "Pac-12", not "Mountain West"). Optional, only when the data clearly supports it: a standout stat leader, conference-race standings, or a one-line what's-next (only when the latest snapshot is labeled "POST-${weekLabel.toUpperCase()}").`,
    ``,
