...
export function buildWeekRecapPrompt(dynasty, year, week) {
  ...
  return [
    ...
    ``,
    `HARD ROTATION RULE: across all distinguishing-detail callouts in the recap, you must use at least 3 DIFFERENT sources from the list above. If you find yourself reaching for the "prior-year postseason cue" (e.g. "last year's CFP X", "a year after the title game") for 5+ teams in a single recap, that is the new failure mode — one ROTATION SOURCE being applied robotically to every team. Rewrite at least half of those to use coaching tenure, quality-wins tally, current-season streak, or rivalry framing. Variety of source is required; not just variety of phrasing within one source.`,
    ``,
    ...
    `  17. ROTATION SOURCE diversity — go back through every distinguishing-detail callout in my recap. How many different ROTATION SOURCES from RULE B did I use (postseason cue, coaching tenure, recruiting, quality-wins, record-quality, rivalry, prior-year ranking, last-meeting)? If 5+ of my callouts use the same source (typically "last year's CFP X" / "a year removed from"), I'm applying one source robotically — rewrite at least half of those to use a different source.`,
    `  18. Nickname collisions — did I use a bare shared nickname ("Tigers", "Bulldogs", "Wildcats", "Cardinals", "Cougars") for more than one team in the recap? If so, the second-onward references must use the program name, city, or full team name. "The Tigers won, the Tigers lost" referring to two different programs is a reader-clarity failure.`,
    ``,
    `Eighteen checks. If any fails, rewrite the offending paragraph. Do not send a draft that hasn't passed every one.`,
    ``,
    ...
    ...namingRuleLines,
    `═══════════════════════════════════════════════════════════`,
    `NICKNAME DISAMBIGUATION — IN-PROSE CLARITY`,
    `═══════════════════════════════════════════════════════════`,
    `Many programs share a nickname. The shared nicknames that appear most often in FBS recaps and cause reader confusion when used loosely:`,
    `   • Tigers — LSU, Missouri, Auburn, Clemson`,
    `   • Bulldogs — Georgia, Mississippi State, Fresno State`,
    `   • Wildcats — Kentucky, Arizona, Kansas State, Northwestern`,
    `   • Cardinals — Stanford, Louisville, Ball State`,
    `   • Cougars — BYU, Houston, Washington State`,
    `   • Gamecocks / Hurricanes / Trojans / Cowboys — usually unambiguous, but watch for cross-sport recap context`,
    ``,
    `Rule: when two or more teams sharing a nickname appear in the same recap, do NOT use the bare nickname alone for either of them after the first reference. Use the program name ("Missouri", "Mizzou", "the Auburn Tigers"), the city ("Baton Rouge"), or the full team name. The failure mode this prevents: two adjacent sentences reading "...pushed the Tigers to 11-0" (Clemson) and "...knocked the Tigers out of the Top 25" (LSU). The reader has to track which Tigers is which; the prose should not make them.`,
    ``,
    `If exactly one team with a given nickname appears in the recap, this rule does not apply — you can use the bare nickname freely.`,
    ``,
    OUTPUT_FORMAT.trim(),
    ``,
    `═══════════════════════════════════════════════════════════`,
