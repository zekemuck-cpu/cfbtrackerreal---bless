...
export function buildWeekRecapPrompt(dynasty, year, week) {
  ...
  return [
    ...
    ``,
    `AROUND THE RANKINGS is NOT a scoreboard. Do NOT march through the poll in numerical order giving each team its own score-plus-record sentence. Instead: group quiet wins together in one clause and spend your words on the results with actual texture — close scores, conference implications, notable records. If five ranked teams each won comfortably over weak opponents, that's ONE sentence: "Miami, Notre Dame, Oregon, Georgia and Penn State all won by double digits, with all five now unbeaten or one loss." Do not give each a dedicated sentence with score and prior-year context attached.`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    ...
    ``,
    `SUBHEADS — short factual news labels. No verb+thesis, no comma-twist contrast:`,
    `   ✅ "Game of the week: Washington 43, Oregon 36"   ✅ "Top 25 results"   ✅ "Poll movement"   ✅ "Around the country"`,
    `   ✗ "Washington Takes the Week, Oregon Pays for It"   ✗ "The SEC Survives, but It Does Not Separate"`,
    ``,
    `═══════════════════════════════════════════════════════════`,
    ...
    ...(isBowlWeek ? [
      ...
      ``,
      `  1. TOP GAMES — the biggest games of the week: ranked-vs-ranked, top-10 losses, and any upset (ranked team falling to unranked). Lead with the biggest result per the LEDE HIERARCHY in RULE A. Group every consequential ranked game here. MANDATORY: if the defending national champion played this week, their result belongs in this section regardless of their current rank. When a featured game appears in LAST MEETINGS, name it as a rematch and use revenge / repeat-domination framing. When a team has notable prior-year context, weave it in ONCE per team if it lands naturally.`,
      `  2. AROUND THE RANKINGS — the rest of the ranked teams' results. A paragraph or two covering what happened to ranked teams not already in TOP GAMES. Where prior-year context tightens the story, use it — but don't force it on every team. No need to mention every ranked team; pick the ones with something worth saying and skip the rest.`,
      `  3. POLL MOVEMENT — if (and only if) the EVOLUTION section shows a clear trajectory across recent weeks. Plain-language characterization per RULE D — no "saga", no "collapse", no "freefall". Each slot 1-25 has exactly one team; never describe ties.`,
    ...
    ``,
    `STRUCTURAL RULES:`,
    `- Each game appears in EXACTLY ONE section. If you mention Washington-Oregon in TOP GAMES, do not mention it again in AROUND THE RANKINGS or AROUND THE COUNTRY.`,
