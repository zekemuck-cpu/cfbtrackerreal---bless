...
export default function GameEdit() {
  ...
  const [formData, setFormData] = useState({
    ...
    scoreGraphic: '',       // legacy: single score-graphic URL (mirrors the shown slot)
    scoreGraphics: {},      // { team1?, neutral?, team2? } — up to three score graphics
    scoreGraphicShown: '',  // which slot the game overview displays
  })
  ...
  const editorPhotoImages = [
    ...GRAPHIC_SIDES.map(s => formData.scoreGraphics?.[s]).filter(Boolean).map(url => ({ url, isGraphic: true })),
    ...((formData.photos || []).map(u => ({ url: u, isGraphic: false }))),
  ].filter((x, i, arr) => x.url && arr.findIndex(y => y.url === x.url) === i)

  // Build the persisted score-graphic fields from the three-slot editor state:
  // the slot map (non-empty slots only), the resolved "shown in overview" side,
  // and a legacy `scoreGraphic` mirror = the shown URL so older readers (game
  // overview, lightbox, photo gallery) keep working without per-slot awareness.
  const buildScoreGraphicsSave = () => {
    const src = formData.scoreGraphics || {}
    const graphics = {}
    for (const s of GRAPHIC_SIDES) if (src[s]) graphics[s] = src[s]
    const sides = GRAPHIC_SIDES.filter(s => graphics[s])
    const shownSide = (formData.scoreGraphicShown && graphics[formData.scoreGraphicShown])
      ? formData.scoreGraphicShown
      : (sides[0] || '')
    const shownUrl = shownSide ? graphics[shownSide] : (formData.scoreGraphic || '')
    return { graphics, hasGraphics: sides.length > 0, shownSide, shownUrl }
  }
  ...
  useEffect(() => {
    if (existingGame) {
      ...
      setFormData({
        ...
        // Three-slot score graphics. Migrate a legacy single graphic into the
        // neutral slot so it stays visible in the overview after re-save.
        scoreGraphics: (existingGame.scoreGraphics && typeof existingGame.scoreGraphics === 'object')
          ? { ...existingGame.scoreGraphics }
          : (existingGame.scoreGraphic ? { neutral: existingGame.scoreGraphic } : {}),
        scoreGraphicShown: existingGame.scoreGraphicShown
          || (existingGame.scoreGraphics ? '' : (existingGame.scoreGraphic ? 'neutral' : '')),
      })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const gameData = {
        ...
        photoTags: prunePhotoTags(formData.photoTags, [...(formData.photos || []), ...GRAPHIC_SIDES.map(s => formData.scoreGraphics?.[s])].filter(Boolean)),
        // Score graphics — up to three (team1 / neutral / team2). `scoreGraphic`
        // mirrors the slot shown in the overview so legacy readers keep working.
        ...(() => {
          const { graphics, hasGraphics, shownSide, shownUrl } = buildScoreGraphicsSave()
          return {
            ...(hasGraphics ? { scoreGraphics: graphics } : {}),
            ...(shownSide ? { scoreGraphicShown: shownSide } : {}),
            ...(shownUrl ? { scoreGraphic: shownUrl } : {}),
          }
        })(),
      }
  ...
  return (
    <div className="space-y-5">
      ...
      {(() => {
        ...
        // Three-slot score graphics: the slot map, which sides have an image,
        // and the side currently shown in the game overview (explicit choice,
        // else the first uploaded slot).
        const sgMap = formData.scoreGraphics || {}
        const uploadedSides = GRAPHIC_SIDES.filter(s => sgMap[s])
        const shownSide = (formData.scoreGraphicShown && sgMap[formData.scoreGraphicShown])
          ? formData.scoreGraphicShown
          : (uploadedSides[0] || '')
          ...
          <Card>
            ...
            {hasScores && (
              <label className="flex items-center gap-2 mb-2 min-w-0">
                ...
                <select
                  value={shownSide}
                  onChange={(e) => setFormData(prev => ({ ...prev, scoreGraphicShown: e.target.value }))}
                  className="flex-1 min-w-0 bg-surface-2 border border-surface-4 rounded-md px-2 py-1 text-xs text-txt-primary focus:outline-none"
                >
            ...
            {!hasScores ? (
              ...
              <div className="space-y-3">
                {/* Team selector — also picks which of the three graphic slots
                    you're uploading to (team1 / neutral / team2). A dot marks a
                    slot that already has a graphic. */}
                <div>
                ...
                <div>
                  ...
                  <ImageUpload
                    ...
                    onChange={(url) => setFormData(prev => {
                      ...
                      // Auto-select the overview slot to the FIRST graphic uploaded;
                      // once the user has chosen one it sticks until they switch.
                      let shown = prev.scoreGraphicShown
                      if (url && (!shown || !next[shown])) shown = activeSide
                      if (!shown || !next[shown]) shown = GRAPHIC_SIDES.find(s => next[s]) || ''
                      return { ...prev, scoreGraphics: next, scoreGraphicShown: shown }
                    })}
