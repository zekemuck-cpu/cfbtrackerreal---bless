...
export default function Recruiting() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Class Arc — chronological strip of class scores across years.
          Each tile is one season; the bar height inside the tile is
          proportional to that year's class score relative to the team's
          all-time max. Selected year is highlighted. Tap-through to
          switch year. Only renders when there are 2+ comparable years. */}
      {!isAllSeasons && classHistory.length >= 2 && (() => {
        const maxScore = Math.max(...classHistory.map(r => r.score || 0), 1)
        const sorted = [...classHistory].sort((a, b) => a.year - b.year)
        return (
          <div className="media-card overflow-hidden">
            <div className="px-3 py-3 sm:px-4 sm:py-4">
              <div className="flex items-baseline justify-between mb-3 gap-2">
                <div className="text-[10px] font-bold uppercase text-txt-tertiary" style={{ letterSpacing: '2px' }}>
                  Class Arc
                </div>
                <div className="text-[10px] font-semibold text-txt-tertiary tabular-nums" style={{ letterSpacing: '1px' }}>
              ...
              <div className="flex items-end gap-1 sm:gap-1.5 overflow-x-auto pb-1">
                {sorted.map(row => {
                  ...
                  const heightPct = Math.max(8, Math.round(((row.score || 0) / maxScore) * 100))
                  return (
                    <button
                      key={row.year}
                      type="button"
                      onClick={() => handleYearChange(row.year)}
                      className="flex-shrink-0 flex flex-col items-center gap-1 rounded-md transition-colors hover:bg-surface-3 px-1.5 py-1.5"
                      style={{ minWidth: '46px' }}
                      title={`${row.year}${row.rank ? ` · #${row.rank}` : ''} · ${formatRecruitingClassScore(row.score)} · ${row.count} commits`}
                    >
                      {/* Bar */}
                      <div className="w-7 sm:w-8 flex items-end" style={{ height: '40px' }}>
                        <div
                          className="w-full rounded-sm transition-colors"
                          style={{
                            height: `${heightPct}%`,
                            backgroundColor: isSelected ? 'var(--text-primary)' : 'var(--surface-5)',
                          }}
                        />
                      </div>
                      <div
                      ...
                      {row.rank && (
                        <div
                          ...
                          style={{
                            fontSize: '9px',
                            fontWeight: 700,
                            color: row.rank <= 10 ? 'var(--accent-warning)' : 'var(--text-tertiary)',
                          }}
                        >
                          #{row.rank}
                        </div>
