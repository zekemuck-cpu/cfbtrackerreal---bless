...
const dictionaries = [
    {
        gameYear: 26,
        dictionary: fs.readFileSync(path.join(__dirname, '../../../../data/zstd-dicts/26/dict.bin'))
    },
    {
        gameYear: 26,
        dictionary: fs.readFileSync(path.join(__dirname, '../../../../data/zstd-dicts/cfb27/dict.bin'))
    }
];
const isonProcessors = new Map();

function getIsonProcessor(gameYear) {
    if (!isonProcessors.has(gameYear)) {
        isonProcessors.set(gameYear, new IsonProcessor(gameYear));
    }
    return isonProcessors.get(gameYear);
}
...
FranchiseZstdTable3FieldStrategy.getFormattedValueFromUnformatted = (
    ...
    const frame = unformattedValue.subarray(zstdDataStartIndex, zstdDataStartIndex + length);
    let isonBuf;
    let gameYear = 26;
    for (const candidate of dictionaries) {
        try {
            isonBuf = zlib.zstdDecompressSync(frame, { dictionary: candidate.dictionary });
            gameYear = candidate.gameYear;
            break;
        } catch {
            // Try the next known game dictionary.
        }
    }
    if (!isonBuf) {
        // Wrong/unknown zstd dictionary. Fail soft so the rest of the file still parses.
        return '{"__undecoded__":true}';
    }

    // Convert the ISON buffer to a JSON object using the class instance
    const jsonObj = getIsonProcessor(gameYear).isonVisualsToJson(isonBuf);
...
FranchiseZstdTable3FieldStrategy.setUnformattedValueFromFormatted = (
    ...
    let originalGameYear = 26;

    // Create the zstd-compressed buffer
    let dictionary = dictionaries[0].dictionary;
    for (const candidate of dictionaries) {
        try {
            zlib.zstdDecompressSync(
                oldUnformattedValue.subarray(
                    FranchiseZstdTable3FieldStrategy.getZstdDataStartIndex(oldUnformattedValue),
                    FranchiseZstdTable3FieldStrategy.getZstdDataStartIndex(oldUnformattedValue) + oldUnformattedValue.readUInt16LE(0)
                ),
                { dictionary: candidate.dictionary }
            );
            dictionary = candidate.dictionary;
            originalGameYear = candidate.gameYear;
            break;
        } catch {
            // Keep looking for the dictionary used by the original value.
        }
    }

    let isonBuf = getIsonProcessor(originalGameYear).jsonVisualsToIson(jsonObj);

    let compressedBuf = zlib.zstdCompressSync(isonBuf, { dictionary });
