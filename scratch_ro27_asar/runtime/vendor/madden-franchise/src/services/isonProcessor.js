...
export class IsonProcessor {
    ...
    loadGameYearData() {
        ...
        if (!IsonProcessor.internedLookups[this.gameYear]) {
            ...
            } else if (this.gameYear === 27) {
                const cfbLookup = this.loadCfb27InternedStringLookup();
                if (cfbLookup) {
                    IsonProcessor.internedLookups[this.gameYear] = cfbLookup;
                } else {
                    IsonProcessor.internedLookups[this.gameYear] =
                        this.loadGameYearDataStatic(26);
                }
            } else {
    ...
    loadCfb27InternedStringLookup() {
        const candidates = [
            process.env.CFB27_STRING_INTERN_PATH,
            process.env.CFB27_DYNASTY_FILES_ROOT
                ? path.join(
                      process.env.CFB27_DYNASTY_FILES_ROOT,
                      'dynasty-string-intern.BIN'
                  )
                : null,
            path.join(
                __dirname,
                '../../data/cfb27/Dynasty_Files/dynasty-string-intern.BIN'
            ),
            path.join(
                __dirname,
                '../../../../../data/cfb27/Dynasty_Files/dynasty-string-intern.BIN'
        ...
        for (const candidate of candidates) {
            ...
            return this.parseInternedStringBin(fs.readFileSync(candidate));
        }
        return null;
    }

    parseInternedStringBin(data) {
        const lookup = {};
        let offset = 0;
        while (offset + 6 <= data.length) {
            const key = data.readUInt16LE(offset);
            const length = data.readUInt32LE(offset + 2);
            offset += 6;
            if (length > data.length - offset) {
                break;
            }
            lookup[key] = data.subarray(offset, offset + length).toString('utf8');
            offset += length;
        }
        return lookup;
    }
    ...
    populateReverseStringLookup() {
        ...
        if (this.stringLookup) {
            for (let key in this.stringLookup) {
                this.reverseStringLookup[this.stringLookup[key].toLowerCase()] =
                    parseInt(key);
            }
        }
    }
    ...
    jsonVisualsToIson(jsonObj) {
        ...
        return this.gameYear >= 26
            ? isonBuffer
            : this.writeTable3IsonData(isonBuffer);
    }
    ...
    writeString(buffer, offset, value) {
        if (
            ...
            offset = this.writeByte(
                buffer,
                offset,
                IsonProcessor.ISON_INTERNED_STRING
            ); // Write interned string type
    ...
    jsonToIson(json, buffer, offset = 0) {
        if (typeof json === 'object' && !Array.isArray(json)) {
            offset = this.writeByte(
                buffer,
                offset,
                IsonProcessor.ISON_OBJECT_START
            ); // Write object start byte
            for (const key in json) {
                offset = this.writeByte(
                    buffer,
                    offset,
                    IsonProcessor.ISON_KEYVALUEPAIR
                ); // Write key-value pair marker
            ...
            offset = this.writeByte(
                buffer,
                offset,
                IsonProcessor.ISON_OBJECT_END
            ); // Write object end byte
        } else if (Array.isArray(json)) {
            offset = this.writeByte(
                buffer,
                offset,
                IsonProcessor.ISON_ARRAY_START
            ); // Write array start byte
            ...
            offset = this.writeByte(
                buffer,
                offset,
                IsonProcessor.ISON_ARRAY_END
            ); // Write array end byte
    ...
    readBytes(length) {
        const bytes = this.fileData.subarray(
            this.isonOffset,
